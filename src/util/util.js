...
const Utils = {
  ...
  isNextgenValidatorEnabled(hook) {
    const result = hook.alter('isFeatureEnabled', 'NEXTGEN_VALIDATOR');
    if (typeof result === 'boolean') {
      return result;
    }
    const value = process.env.USE_NEXTGEN_VALIDATOR;
  ...
  ObjectId(id) {
    try {
      return _.isObject(id) ? id : new ObjectID(id);
    } catch (ignoreErr) {
      return id;
    }
  },
