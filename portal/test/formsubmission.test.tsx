...
test('Typing in enter data auto-saves a draft submission before submit', async () => {
  const draftPosts: Array<{ state?: string; data?: { textField?: string } }> = [];
  server.use(
    http.get('http://localhost:3002/current', () => {
      return HttpResponse.json({ _id: '679d116aa90ca7ccebc38501', data: { email: 'user@example.com' } });
    }),
    http.get('http://localhost:3002/form', ({ request }) => {
      const queryParameters = new URL(request.url).searchParams;
      if (queryParameters.get('type') === 'form') {
    ...
    http.get('/form/679d116aa90ca7ccebc38597/submission', () => {
      return HttpResponse.json([]);
    }),
    http.post('/form/679d116aa90ca7ccebc38597/submission', async ({ request }) => {
      const body = (await request.json()) as { state?: string; data?: { textField?: string } };
      draftPosts.push(body);
      return HttpResponse.json({
        _id: '679d116aa90ca7ccebc39999',
        state: body.state || 'draft',
        data: body.data || {},
      });
    }),
    http.post('http://localhost:3002/form/679d116aa90ca7ccebc38597/submission', async ({ request }) => {
      const body = (await request.json()) as { state?: string; data?: { textField?: string } };
      draftPosts.push(body);
      return HttpResponse.json({
        _id: '679d116aa90ca7ccebc39999',
  ...
  await userEvent.click(await screen.findByText('test'));
  await screen.findByText('Form Title');
  await userEvent.click(await screen.findByText('Enter Data'));
  const input = await screen.findByLabelText('Text Field');
  await userEvent.type(input, 'hello');
  await waitFor(
    () => {
      expect(draftPosts.length).to.be.greaterThan(0);
      expect(draftPosts.some((post) => post.state === 'draft' && post.data?.textField === 'hello')).to
        .be.true;
    },
    { timeout: 4000 },
  );
